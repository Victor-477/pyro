...
<!-- bench_vm:begin — regenerated by --save; edits here are lost -->
...
> **`--save` rewrites only the region between the two markers above** — the
> machine block and the table. Everything else on this page is written by hand
> and is left alone.
>
> It did not always work that way. `--save` used to rebuild the whole file and
> open it with `'w'`, so each run **deleted every word below the table**: once,
> 452 lines down to 34, taking the entire 13.4 write-up with it. The safeguard
> was a paragraph here asking you to copy the prose out and paste it back, which
> safeguards nothing the first time someone does not read it. If the markers go
> missing, `--save` now refuses and says so rather than writing.
