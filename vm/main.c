...
// Global VM state (for stack traces and fatal error reporting)
            ...
            case opKEYS:
                {
                    ...
                    int64_t count = 0;
                    MapPair* pairs = malloc(sizeof(MapPair) * mp.as.map->size);
                    for (int64_t i = 0; i < mp.as.map->capacity; i++) {
                        MapNode* curr = mp.as.map->buckets[i];
                        while (curr) {
                            pairs[count].key = curr->key;
                            pairs[count].val = curr->val;
                            pairs[count].key_str = value_to_string(curr->key);
